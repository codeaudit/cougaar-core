...
public class PluginBinder extends BinderSupport implements PluginBindingSite
{
  ...
  protected final PlugInAdapter getPlugin() {
    return (PlugInAdapter) getChildComponent();
  }
  protected final PluginManager getPluginManager() {
    return (PluginManager)getParentComponent();
  }

    // parameters belong in plugin adapter - there is a new method
    // for getting them anyway.
  /*
  ...
  */
  ...
  protected abstract class Threading implements GenericStateModel {
    ...
    public String toString() {
      return getClusterIdentifier()+"/"+(PluginBinder.this);
    }
  }
  ...
  protected class SharedThreading extends Threading implements ScheduleablePlugIn {
    ...
    public final void addExternalActivityWatcher(SubscriptionWatcher watcher) { 
      (getPlugin().getBlackboardService()).registerInterest(watcher);      
    }
  ...
  protected class SingleThreading extends Threading implements Runnable {
    ...
    public void load(Object object) {
      waker = getPlugin().getBlackboardService().registerInterest();
    }
    public void start() {
      myThread = new Thread(this, "Plugin/"+getClusterIdentifier()+"/"+(PluginBinder.this));
